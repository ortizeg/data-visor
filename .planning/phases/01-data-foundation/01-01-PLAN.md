...
must_haves:
  ...
  artifacts:
    - path: "app/main.py"
      ...
      contains: "lifespan"
    - path: "app/config.py"
      provides: "Pydantic Settings with env prefix DATAVISOR_"
...
<objective>
Set up the DataVisor Python project with FastAPI, DuckDB connection management, schema creation, Pydantic data models, and test infrastructure.
...
<task type="auto">
  ...
  <action>
    ...
    Create `.env.example` with all DATAVISOR_ prefixed env vars documented:
    ...
    - DATAVISOR_PORT=8000
    - DATAVISOR_GCS_CREDENTIALS_PATH= (optional, empty default)
...
<task type="auto">
  ...
  <action>
    **app/config.py:** Create Pydantic Settings class with fields:
    ...
    - Use env_prefix="DATAVISOR_" and env_file=".env"
