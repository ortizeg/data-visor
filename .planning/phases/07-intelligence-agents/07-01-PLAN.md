...
must_haves:
  truths:
    ...
    - "Agent model is configurable via DATAVISOR_AGENT_MODEL env var"
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `app/routers/agent.py`:
       ...
       - POST `/{dataset_id}/analyze` endpoint:
         ...
         - Handle ValueError (no API key) -> 503 with clear message "Configure DATAVISOR_AGENT_MODEL and corresponding API key"
  ...
  <done>
    - Config has agent_model and vlm_device settings with DATAVISOR_ prefix
