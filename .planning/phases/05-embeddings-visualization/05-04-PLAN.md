...
<task type="auto">
  ...
  <action>
    ...
    **Architecture note -- lasso bypasses filter-store:**
    Lasso selection is a cross-filter concern (spatial selection from the embedding space), NOT a metadata filter (like category, split, or tag). Therefore, `use-samples.ts` reads `lassoSelectedIds` directly from `embedding-store.ts`, NOT from `filter-store.ts`. The filter-store remains exclusively for metadata/annotation filters. This keeps the two filter domains cleanly separated -- metadata filters compose in filter-store, spatial selection lives in embedding-store, and use-samples.ts is the single integration point that reads from both stores and combines them into the API query.

    - Import `useLassoSelectedIds` from `@/stores/embedding-store`
    - Read `lassoSelectedIds` from the embedding store (NOT from filter-store)
    - Include in queryKey: add `lassoSelectedIds` to the query key array (as a separate key element, not inside the filters object). When lassoSelectedIds changes, TanStack Query auto-refetches.
    ...
    - **Do NOT modify `filter-store.ts`** -- it has no role in lasso selection.
  </action>
  <verify>
